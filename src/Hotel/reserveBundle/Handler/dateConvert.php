...
class dateConvert
{
    ...
    static function jdate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa')
    {
        ...
        $ts = $T_sec + (($timestamp == '' or $timestamp == 'now') ? time() : dateConvert::tr_num($timestamp));
        $date = explode('_', date('H_i_j_n_O_P_s_w_Y', $ts));
        list($j_y, $j_m, $j_d) = dateConvert::gregorian_to_jalali($date[8], $date[3], $date[2]);
    ...
    {
        $g_y = dateConvert::tr_num($g_y);
        $g_m = dateConvert::tr_num($g_m);
        $g_d = dateConvert::tr_num($g_d); /* <= :اين سطر ، جزء تابع اصلي نيست */
        $d_4 = $g_y % 4;
